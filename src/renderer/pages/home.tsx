...
const HomePage = () => {
  ...
  return (
    <Layout>
      <Header>
        <h1>{t("home_page_games")}</h1>

        <Button icon="add" to="/games/new/settings">
          {t("home_page_add_game")}
        </Button>
      </Header>

      {validGames.length > 0 && (
        <GamesContainer>
          <h2>{t("home_page_ready_games")}</h2>
          <Games>
            {validGames.map((game) => (
      ...
      {notValidGames.length > 0 && (
        <GamesContainer>
          <h2>{t("home_page_your_saves_library")}</h2>
          <Games>
      ...
      {installedGames.length > 0 && (
        <GamesContainer>
          <h2>{t("home_page_installed_games")}</h2>
          <Games>
      ...
      {validGames.length === 0 && notValidGames.length === 0 && (
        <NoGames>{t("home_page_no_games_found")}</NoGames>
      )}
    </Layout>
  );
};
